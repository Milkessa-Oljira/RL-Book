...
def iterate_batches(envs: tt.List[gym.Env], 
    ...
    while True:
        ...
        if len(batch) == batch_size:
            ...
            yield torch.tensor(batch_np * 2. / 255. - 1.)
            batch.clear()
...
if __name__ == "__main__":
    ...
    for batch_v in iterate_batches(envs):
        ...
        if iter_no % SAVE_IMAGE_EVERY_ITER == 0:
            img = utils.make_grid(gen_output_v.data[:64], normalize=True)
            writer.add_image("fake", img, iter_no)
            img = utils.make_grid(batch_v.data[:64], normalize=True)
